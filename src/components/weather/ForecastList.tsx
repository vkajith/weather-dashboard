    ...
    <Card title={<span className="gradient-text text-2xl font-bold">5-Day Forecast</span>}>
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
        {data.forecast.map((item: ForecastItem, index: number) => (
          <div
            key={index}
            className="bg-white/5 backdrop-blur-md rounded-xl p-4 transform transition-all duration-300 hover:scale-105"
          >
            <div className="font-medium text-white mb-2">{formatDate(item.date)}</div>
            <div className="weather-icon-container mb-2">
              <img
            ...
            <div className="text-sm text-blue-200/80 mb-3">{item.condition}</div>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-white/5 backdrop-blur-md rounded-lg p-2">
                <div className="text-blue-200/60 text-xs mb-1">Min</div>
                <div className="text-white font-medium">{Math.round(item.minTemp)}°</div>
              </div>
              <div className="bg-white/5 backdrop-blur-md rounded-lg p-2">
                <div className="text-blue-200/60 text-xs mb-1">Max</div>
